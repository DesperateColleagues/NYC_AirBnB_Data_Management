...
SELECT count_compliances(TRUE) 
			AS compliant_bnb_without_licence, 
       count_compliances(FALSE) 
	   		AS not_compliant_bnb_without_licence,
	   (SELECT COUNT(*) FROM rental_units) - (count_compliances(TRUE) + count_compliances(FALSE)) 
...
CREATE OR REPLACE VIEW ideal_rental_unit_per_band_rate AS (
	WITH ideal_rental_unit_per_rate AS (
		SELECT rate, 
			   ...
			   CAST(AVG(rf.minimum_nights) AS INTEGER) AS avg_min_nights,
	...
	SELECT  rental_unit_band,
			...
			CAST(AVG(avg_min_nights) AS INTEGER) AS avg_min_nights,
